"""
This module contains classes and methods for setting the molecule driver and executing the VQE algorithm
...
class EnergyEstimator:
    ...
    3. Initial state: This is the initial state of the qubits that can be passed to an Ansatz. Usually, 
        the initial state would be Hartree-Fock state as that works well for estimating ground state energy
        problems.
    ...
    6. Mapping electronic structure problems to qubits: For fermionic problems, 3 types of mappers 
        ...
        We have used the parity mapper as default along with two-qubit-reduction as it helps in 
        reducing the number of required qubits when mapping the 2nd quantized hamiltonian.
    """    
    
    # define constructor
    def __init__(self, geometry, z2symmetry_reduction=None, orbital_reduction_list=None, mapper='P', 
                    charge=0, unit=UnitsType.ANGSTROM, basis='sto3g', debug=False) -> None:
        """
        ...
        self.molecule = Molecule(geometry=geometry, charge=charge, units=unit)
        ...
        try:
            if mapper == 'P': # P indicates parity mapper
                ...
                self.qubit_converter = QubitConverter(mapper=JordanWignerMapper(), two_qubit_reduction=False, 
                                            z2symmetry_reduction=z2symmetry_reduction)
            else:
        ...
        # Apply transformations to the driver to get the electronic structure problem
    ...
    def get_hamiltonian_op(self, debug=False):
        ...
        second_quantized_op = self.electronic_structure_problem.second_q_ops()
        if debug == True:
            log.debug(f"Printng second quantized hamiltonian ...")
            log.debug(second_quantized_op)
    
        self.qubit_hamiltonian_op = self.qubit_converter.convert(
                                        second_quantized_op["ElectronicEnergy"], 
        ...
        return self.qubit_hamiltonian_op

    def get_qubit_op_strings_and_coeffs(self, debug=False):
        """
        Description: This method separates out the pauli strings and their corresponding
        coefficients. The pauli strings are returned in the form of list of strings, 
        whereas coefficients are returned in the form of Array.
        """

        log.info(f"Extracting pauli operator strings and coefficients from qubit hamiltonian ...")
        pauli_op_list = self.qubit_hamiltonian_op.primitive.to_list()
        if debug == True:
            log.debug(f"Printing Pauli operator list with coefficients ...")
            log.debug(pauli_op_list)
    ...
    def build_ansatz(self, num_qubits, initial_state, rotations, entanglement, 
        ...
        # add the initial state to ansatz
...
def variational_eigen_solver(ansatz, optimizer, quantum_instance):
    """
...
def single_plot_graph(x_values, y_values, plot_title, x_label, y_label, debug=False):
...
def multiple_plot_graph(x_values, y_values, plot_title, x_label, y_label, debug=False):
    ...
    for key in y_values:
        ...
        if key == 'VQE':
            annot_min(x_values, y_values[key], ax)
